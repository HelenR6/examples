...
with torch.no_grad():
    ...
    for i, (images, target) in enumerate(tqdm(loader)):
        ...
        with torch.enable_grad():
            ...
            res_adv_untargeted=resnet_adversary.perturb(images, target)
        diff=adv_untargeted-images
        print(diff.shape)
        diff_norm=torch.norm(diff,p=2,dim=(1,2,3))
        print(diff_norm)
        resnet_diff=res_adv_untargeted-images
        resnet_diff_norm=torch.norm(resnet_diff,p=2,dim=(1,2,3))
        print(resnet_diff_norm)
        # predict
        image_features = model.visual(adv_untargeted)
